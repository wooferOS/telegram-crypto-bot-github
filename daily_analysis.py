...
def generate_gpt_report(wallet_text):
    ...
    prompt = f"""
Ти — досвідчений GPT-криптоасистент, що аналізує криптопортфель користувача. 

🔹 Використовуй ТІЛЬКИ USDT і гривню ₴.
🔹 Не пиши символ $. Не пиши фрази «я лише припускаю», «зробіть власне дослідження».
🔹 Дай чіткі рекомендації на продаж і купівлю з командами.
🔹 По кожному активу: назва, кількість, ціна в USDT, еквівалент у гривні, стоп-лосс, тейк-профіт.
🔹 Пиши прогноз на кінець доби. Формат обовʼязково:

📊 Звіт на {today}

🔻 ПРОДАЖ:
1. <MONETA> — <КІЛЬКІСТЬ> ≈ <USDT> ≈ <₴>  
Причина: ...  
/confirmsell<MONETA>
...
🧠 Прогноз: які монети мають перспективу зростання, які — в зоні ризику. Теханаліз має базуватись на Binance Academy, ринку Binance: https://www.binance.com/uk-UA/markets/overview

Ось криптопортфель користувача:
{wallet_text}
"""
    ...
    response = client.chat.completions.create(
        model="gpt-4",
        messages=[
            {"role": "system", "content": "Ти GPT-криптоаналітик. Формуй короткі, точні, впевнені звіти по Binance-портфелю без зайвого тексту."},
