...
import os, csv, sys, fcntl, tempfile

CSV_FIELDS = [
    "ts","region","phase","from","to","wallet","amount",
    "ratio","toAmount","available","insufficient","ok","quoteId","error"
]
...
def _parse_cli_region_phase(argv: List[str] | None = None) -> Tuple[str,str]:
    ...
    for i, a in enumerate(argv):
        if a == "--region" and i+1 < len(argv): region = argv[i+1]
        if a == "--phase"  and i+1 < len(argv): phase  = argv[i+1]
        if a.startswith("--region="): region = a.split("=",1)[1]
        if a.startswith("--phase="):  phase  = a.split("=",1)[1]
...
def _dedup_rows(existing: List[dict], new_rows: List[dict]) -> List[dict]:
    ...
    def key(r): return (r.get("from"), r.get("to"), r.get("wallet"), str(r.get("amount")))
...
def write_reports(items: Iterable[Mapping[str, Any]], outdir: str | Path | None = None) -> str:
    ...
    with lock_path.open("w") as lk:
        ...
        def _b(s): return str((s or "")).strip().lower() == "true"
        def _agg(rs):
            return {
                "total": len(rs),
